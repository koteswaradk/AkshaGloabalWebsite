...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `px-3 py-1.5 rounded-full text-sm font-medium transition-colors duration-200 inline-flex items-center gap-1.5 ${
      isActive
        ...
        <div className="flex items-center justify-between h-16">
          ...
          >
            ...
            <span className="text-xl font-bold text-m3-on-surface dark:text-m3-dark-on-surface">
              Aksha<span className="text-m3-primary dark:text-m3-dark-primary"> Globals{isStudio ? ' Studios' : isKmp ? ' KMP' : ''}</span>
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1.5">
            {navLinks.map(link => (
              <NavLink key={link.to} to={link.to} className={linkClass} end={link.to === '/'}>
                ...
                {link.badge && (
                  <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-full bg-cyan-600/20 text-cyan-400 border border-cyan-500/30">
                    {link.badge}
                  </span>
                )}
              </NavLink>
        ...
        <div className="md:hidden bg-m3-surface-container dark:bg-m3-dark-surface-container border-t border-m3-outline-variant dark:border-m3-dark-outline px-4 py-3 flex flex-col gap-1">
          {navLinks.map(link => (
            <NavLink
              ...
              to={link.to}
              className={({ isActive }) =>
                `flex items-center justify-between px-3 py-2 rounded-full text-sm font-medium transition-colors duration-200 ${
                  isActive
                    ? 'text-m3-on-primary-container dark:text-m3-dark-on-primary-container bg-m3-primary-container dark:bg-m3-dark-primary-container'
            ...
            >
              <span>{link.label}</span>
              {link.badge && (
                <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-full bg-cyan-600/20 text-cyan-400 border border-cyan-500/30">
                  {link.badge}
                </span>
              )}
            </NavLink>
