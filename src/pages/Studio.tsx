...
const sliderData: Array<{
...
}> = [
  {
    ...
    subtitle: 'Captivating stories, rhymes and devotional media productions created for audiences of all ages.',
    category: null,
    gradient: 'linear-gradient(135deg, #111827 0%, #1F2937 50%, #0F172A 100%)',
    isHero: true,
    bannerImage: `${baseUrl}slider-images/slide3.png`,
    bannerWebp: `${baseUrl}slider-images/slide3.webp`,
    tag: 'Welcome to Aksha Globals Studios',
  },
  {
    title: 'Captivating Animated Stories',
    subtitle: 'Heartwarming animated tales that spark imagination, curiosity, and instill valuable moral character.',
    category: 'stories' as const,
    gradient: 'linear-gradient(135deg, #0F172A 0%, #1e3a5f 50%, #111827 100%)',
    bannerImage: `${baseUrl}slider-images/slide3.png`,
    bannerWebp: `${baseUrl}slider-images/slide3.webp`,
    tag: 'Stories & Moral Tales',
  },
  {
    title: 'Joyful Nursery Rhymes',
    subtitle: 'Vibrant animated nursery melodies that children love to dance, chant, and sing along with.',
    category: 'rhymes' as const,
    gradient: 'linear-gradient(135deg, #1E1B4B 0%, #2e1065 50%, #111827 100%)',
    bannerImage: `${baseUrl}slider-images/slide3.png`,
    bannerWebp: `${baseUrl}slider-images/slide3.webp`,
    tag: 'Music & Rhymes',
  },
  {
    ...
    subtitle: 'Sacred chants, stotras, and spiritual bhajans providing tranquility, peace, and meditation.',
    category: 'devotional' as const,
    gradient: 'linear-gradient(135deg, #431407 0%, #7c2d12 50%, #111827 100%)',
    bannerImage: `${baseUrl}slider-images/slide3.png`,
    bannerWebp: `${baseUrl}slider-images/slide3.webp`,
    tag: 'Spiritual Chants & Bhajans',
  },
]
...
export default function Studio() {
  ...
  const studioTouchStartX = useRef<number | null>(null)
  const studioTouchEndX = useRef<number | null>(null)

  const handleTouchStart = (e: React.TouchEvent) => {
    setIsPaused(true)
    studioTouchStartX.current = e.targetTouches[0].clientX
    studioTouchEndX.current = null
  }

  const handleTouchMove = (e: React.TouchEvent) => {
    studioTouchEndX.current = e.targetTouches[0].clientX
  }

  const handleTouchEnd = () => {
    setIsPaused(false)
    if (studioTouchStartX.current === null || studioTouchEndX.current === null) return
    const diff = studioTouchStartX.current - studioTouchEndX.current
    if (diff > 45) {
      nextSlide()
    } else if (diff < -45) {
      prevSlide()
    }
    studioTouchStartX.current = null
    studioTouchEndX.current = null
  }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#1F2937' }}>
      ...
      >
        <div
          className="transition-all duration-700 relative min-h-[400px] sm:min-h-[460px] lg:min-h-[500px] flex flex-col justify-center"
          style={{ background: currentSlide.gradient }}
        >
          {/* Banner background layer with responsive fit */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {/* Ambient blurred backdrop for uniform tone */}
            <picture>
              <source srcSet={currentSlide.bannerWebp} type="image/webp" />
              <img
            ...
            {/* Sharp responsive banner image positioned for desktop artwork showcase without clipping */}
            <div className="absolute inset-0 flex items-center justify-end">
              <picture className="w-full lg:w-3/5 h-full opacity-30 sm:opacity-45 lg:opacity-75 transition-opacity duration-700">
                <source srcSet={currentSlide.bannerWebp} type="image/webp" />
                <img
                  src={currentSlide.bannerImage}
            ...
            {/* Gradient overlays to guarantee 100% WCAG AA text contrast on all devices */}
            <div
              className="absolute inset-0"
              style={{
                background: 'linear-gradient(to right, rgba(15,23,42,0.96) 0%, rgba(15,23,42,0.85) 45%, rgba(15,23,42,0.4) 100%)',
              }}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-gray-950 via-transparent to-black/30 lg:hidden" />
          </div>

          {/* Decorative ambient elements */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none select-none">
            <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full bg-orange-500/10 blur-3xl" />
            <div className="absolute -bottom-32 -left-16 w-80 h-80 rounded-full bg-orange-500/5 blur-2xl" />
          </div>

          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-18 relative z-10 w-full">
            <div
              key={sliderIndex}
            ...
            >
              ...
              <div className="flex-1 text-center lg:text-left">
                <span
                  className="inline-block text-xs sm:text-sm font-semibold tracking-widest uppercase mb-3 sm:mb-4 px-3.5 py-1 rounded-full backdrop-blur-sm shadow-sm"
                  style={{ color: '#FDBA74', border: '1px solid rgba(249,115,22,0.3)', backgroundColor: 'rgba(249,115,22,0.1)' }}
                >
                  {currentSlide.tag}
                </span>
                ...
                <p
                  className="text-base sm:text-lg md:text-xl mb-6 sm:mb-8 max-w-xl leading-relaxed mx-auto lg:mx-0"
                  style={{ color: '#D1D5DB' }}
                >
                ...
                {!currentSlide.isHero && currentSlide.category && (() => {
                  ...
                  return (
                    <button
                      ...
                      className="px-6 sm:px-8 py-3 sm:py-3.5 font-bold rounded-full transition-all duration-200 shadow-xl hover:shadow-2xl hover:-translate-y-0.5 active:translate-y-0 text-sm sm:text-base cursor-pointer"
              ...
              {/* Right - Visual panel */}
              <div
                className="flex-shrink-0 w-full max-w-sm lg:w-80 xl:w-96"
              ...
              >
                {currentSlide.isHero ? (
                    ...
                    {categories.map(cat => {
                      ...
                      return (
                        <button
                          ...
                          className="rounded-2xl p-3.5 sm:p-5 border text-center transition-all duration-200 hover:-translate-y-1 shadow-lg backdrop-blur-md cursor-pointer hover:border-orange-500/50"
                          style={{
                            backgroundColor: 'rgba(255,255,255,0.08)',
                            borderColor: 'rgba(255,255,255,0.15)',
                          }}
                        >
                          <div className="text-3xl sm:text-4xl mb-1.5 sm:mb-2">{cat.icon}</div>
                          <div className="text-xs sm:text-sm font-bold" style={{ color: '#F9FAFB' }}>{cat.label}</div>
                          <div className="text-[11px] sm:text-xs mt-0.5 sm:mt-1" style={{ color: 'rgba(255,255,255,0.6)' }}>{count} playlists</div>
                        </button>
                      )
                    })}
                  </div>
                  ...
                  (() => {
                    ...
                    return (
                      <div
                        className="rounded-2xl p-5 sm:p-7 border text-center backdrop-blur-md shadow-2xl"
                        style={{
                          backgroundColor: 'rgba(255,255,255,0.08)',
                          borderColor: 'rgba(255,255,255,0.15)',
                        }}
                      >
                        <div className="text-5xl sm:text-6xl md:text-7xl mb-3 sm:mb-4 drop-shadow-xl">{cat?.icon}</div>
                        <div className="text-xl sm:text-2xl font-bold mb-1" style={{ color: '#F9FAFB' }}>{cat?.label}</div>
                        <div className="text-xs sm:text-sm mb-3 sm:mb-4" style={{ color: 'rgba(255,255,255,0.7)' }}>{cat?.description}</div>
                        <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                          <div className="rounded-xl p-2.5 sm:p-3" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
                            <div className="text-[11px] sm:text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.6)' }}>Playlists</div>
                            <div className="font-bold text-xs sm:text-sm" style={{ color: '#F9FAFB' }}>{catPlaylists.length}</div>
                          </div>
                          <div className="rounded-xl p-2.5 sm:p-3" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
                            <div className="text-[11px] sm:text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.6)' }}>Videos</div>
                            <div className="font-bold text-xs sm:text-sm" style={{ color: '#F9FAFB' }}>{totalVideos}</div>
                          </div>
                        </div>
                      </div>
                    )
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 pb-5 sm:pb-6 relative z-10">
            {sliderData.map((_, i) => (
              <button
                key={i}
                onClick={() => goToSlide(i)}
                ...
                className={`transition-all duration-300 rounded-full cursor-pointer ${
                  i === sliderIndex
                    ? 'w-7 sm:w-8 h-2 sm:h-2.5 bg-white shadow-md shadow-white/50'
          ...
          <button
            ...
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/75 backdrop-blur-md border border-white/20 rounded-full w-8 h-8 sm:w-11 sm:h-11 flex items-center justify-center text-white transition-all duration-200 shadow-lg z-20 cursor-pointer hover:scale-105 active:scale-95"
          >
          ...
          <button
            ...
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/75 backdrop-blur-md border border-white/20 rounded-full w-8 h-8 sm:w-11 sm:h-11 flex items-center justify-center text-white transition-all duration-200 shadow-lg z-20 cursor-pointer hover:scale-105 active:scale-95"
          >
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          ...
          <div className="h-1 relative z-10" style={{ background: 'linear-gradient(90deg, #F97316, #1F2937, #F97316)' }} />
